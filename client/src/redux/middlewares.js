...
import { saveNewUserMessage ,  SAVE_FORM_INPUT , LOAD_INPUTS_FROM_LOCAL_STORAGE , RESET_FORM_DATA , SAVE_NEW_USER} from "./actions";
...
export const saveNewUserMiddleware = ({ dispatch , getState }) => next => async action => {
    ...
    try {     
        ...
        if(response.error) {
            if (response.error.keyValue && response.error.keyValue.userName) {
                dispatch(saveNewUserMessage('info', '!שם המשתמש כבר קיים'));
            } else {
